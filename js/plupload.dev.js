	...
	function onUploadFile(up, file) {
		...
		function handleError() {
			if (retries-- > 0) {
				...
				up.trigger('Error', {
					...
					file : file,
					response : file.xhr.responseText,
					status : file.xhr.status,
					responseHeaders: file.xhr.getAllResponseHeaders()
				});
			}
		}
		...
		function uploadNextChunk() {
			...
			if (up.settings.multipart && features.multipart) {
				file.xhr.open("post", url, true);

				// Set custom headers
				plupload.each(up.settings.headers, function(value, name) {
					file.xhr.setRequestHeader(name, value);
				});
				...
				file.xhr.setRequestHeader('Content-Type', 'application/octet-stream'); // Binary stream header
